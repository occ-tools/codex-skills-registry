...
export function summarizeCliJson(data) {
...
export function writeGithubOutputSummary(summary, outputPath = process.env.GITHUB_OUTPUT) {
    if (!outputPath) {
...
export function writeGithubOutputSummaryFromFile(summaryFile, outputPath = process.env.GITHUB_OUTPUT) {
    const data = JSON.parse(readFileSync(summaryFile, "utf8"));
    writeGithubOutputSummary(summarizeCliJson(data), outputPath);
}
function collectIssueArrays(value) {
    const directArrays = [
    ...
    const resultIssues = results.flatMap((result) => isRecord(result) ? arrayValue(result.issues) : []);
    const arrays = directArrays.some((issues) => issues.length > 0) || resultIssues.length > 0
        ? directArrays
...
//# sourceMappingURL=action-summary.js.map