...
import { CodexSkillSchema, McpConfigFileSchema, McpServerConfigSchema, PluginManifestSchema, SkillFrontmatterSchema, } from "./schema.js";
...
const JSON_SCHEMA_DEFINITIONS = [
    {
        ...
        description: "YAML frontmatter accepted in a Codex Skill SKILL.md file by codex-skills-registry.",
    ...
    },
    {
        ...
        description: "Normalized registry entry produced from SKILL.md, config files, plugin bundles, or inline SDK records.",
    ...
    },
    {
        ...
        description: "Project policy file used to make registry validation and MCP audits stricter in CI.",
    ...
    },
    {
        ...
        description: "Codex plugin manifest with registry-supported skill and MCP component references.",
 ...
 */
export function listRegistryJsonSchemaNames() {
 ...
 */
export function isRegistryJsonSchemaName(name) {
    return listRegistryJsonSchemaNames().includes(name);
}
/**
 ...
 */
export function createRegistryJsonSchema(name) {
 ...
 */
export function createRegistryJsonSchemaCatalog() {
    const defs = Object.fromEntries(JSON_SCHEMA_DEFINITIONS.map((definition) => [
    ...
    return {
        ...
        description: "JSON Schemas for Codex Skill frontmatter, normalized registry entries, registry policy files, plugin manifests, and MCP server configuration supported by codex-skills-registry.",
        anyOf: refs,
        $defs: defs,
    };
}
function createJsonSchemaForDefinition(definition) {
    const generated = removeStandardMetadata(z.toJSONSchema(definition.schema, {
...
function toNestedDefinition(schema, definitionName) {
    const { $schema: _schema, $id: _id, ...nestedSchema } = schema;
    return rewriteLocalJsonSchemaRefs(nestedSchema, definitionName);
}
function rewriteLocalJsonSchemaRefs(value, definitionName) {
...
function rewriteLocalJsonSchemaRef(ref, definitionName) {
    return ref.startsWith("#/$defs/")
        ? ref.replace("#/$defs/", `#/$defs/${definitionName}/$defs/`)
        : ref;
}
function removeStandardMetadata(value) {
    if (Array.isArray(value)) {
