...
export async function readCommentArtifactPrefix(filePath: string): Promise<CommentArtifactPrefix> {
...
export function formatUntrustedCommentArtifact(
  source: string,
...
function escapeArtifactPayload(source: string): { value: string; truncated: boolean } {
  const parts: string[] = [];
  let bytes = 0;
...
export async function publishCommentArtifact(
  filePath: string,
  ...
  console.log(
    `${result.updated ? "Updated" : "Created"} pull request comment${result.url ? `: ${result.url}` : "."}`,
  );
}
